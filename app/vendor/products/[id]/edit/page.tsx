  ...
  useEffect(() => {
    Promise.all([
      ...
      if (pd.product) {
        const p = pd.product
        // Pre-fill form
        const existingPhotos = (p.photo_urls as string[]) ?? []
        const paddedPhotos: (string | null)[] = [...existingPhotos, ...Array(5).fill(null)].slice(0, 5)
        setPhotoUrls(paddedPhotos)
        setName(p.name_en as string ?? '')
        setCategoryId(p.category_id as string | null)
        setCondition((p.condition as Condition) ?? 'new')
        setDescription(p.description_en as string ?? '')
        setInStock((p.stock_status as string) === 'in_stock')
        setPrice((p.price as number)?.toLocaleString() ?? '')
        setBargain(p.bargaining_allowed as boolean ?? false)
      }
  ...
  return (
    <main className="min-h-screen pb-28">
      <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-surface-3 bg-background/95 px-4 backdrop-blur">
        ...
        <h1 className="font-syne text-[16px] font-bold text-foreground">Edit Product</h1>
        <div className="w-9" />
      </header>
      ...
      <div className="mx-auto w-full max-w-[640px] space-y-4 px-4 pt-4">
        ...
        <section className="space-y-3 rounded-2xl border border-surface-3 bg-surface-1 p-4">
          ...
          <div className={cn('flex h-12 items-center gap-2.5 rounded-xl border bg-surface-2 px-3', price ? 'border-primary' : 'border-surface-3')}>
            ...
            <input
              value={price}
              onChange={(e) => { const d = e.target.value.replace(/\D/g, ''); setPrice(d ? parseInt(d, 10).toLocaleString() : '') }}
              inputMode="numeric"
              placeholder="0"
              className="h-full w-full bg-transparent text-[14px] text-foreground placeholder:text-muted-foreground focus:outline-none"
            />
        ...
        {error && (
          <p className="rounded-xl border border-error bg-error/10 px-4 py-3 text-[13px] text-error">{error}</p>
        )}
      </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 mx-auto max-w-[640px] border-t border-surface-3 bg-surface-1 p-4">
        <button
          ...
          className={cn(
            ...
            canSubmit ? 'bg-primary text-primary-foreground' : 'cursor-not-allowed bg-surface-3 text-muted-foreground',
          )}
        >
